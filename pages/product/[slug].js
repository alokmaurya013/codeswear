...
const Post = ({buyNow,addToCart,product,variants,error}) => {
  ...
  <div className="container px-5 py-16 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto px-24 object-cover object-top rounded" src={product[0].img}/>

      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        <h2 className="text-sm title-font text-gray-500 tracking-widest">CODESWEAR</h2>
        <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{product[0].title}({product[0].size}/{product[0].color})</h1>
        ...
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          ...
          {Object.keys(variants).includes('white')&& Object.keys(variants['white']).includes(size)&&<button onClick={()=>{refreshVariant(size,'white')}} className={`b order-2 border-gray-300 rounded-full w-6 h-6 focus:outline-none ${color=='white'? 'border-black':'border-gray-300'}`}></button>}
          ...
          {Object.keys(variants).includes('purple')&&Object.keys(variants['purple']).includes(size)&&<button onClick={()=>{refreshVariant(size,'purple')}} className={`border-2 border-gray-300 ml-1 bg-purple-500 rounded-full w-6 h-6 focus:outline-none ${clor=='purple'? 'border-black':'border-gray-300'}`}></button>}          
          ...
          <div className="flex ml-6 items-center">
            ...
            <div className="relative">
              ...
              {color&&variants[color]&&Object.keys(variants[color]).includes('S')&&<option value={'S'}>S</option>}
              {color&&variants[color]&&Object.keys(variants[color]).includes('M')&&<option value={'M'}>M</option>}
              {color&&variants[color]&&Object.keys(variants[color]).includes('L')&&<option value={'L'}>L</option>}
              {color&&variants[color]&&Object.keys(variants[color]).includes('XL')&&<option value={'XL'}>XL</option>}
              {color&&variants[color]&&Object.keys(variants[color]).includes('XXL')&&<option value={'XXL'}>XXL</option>}
              </select>
        ...
        <div className="flex ">
          {product[0].availableQty>0&&<span className="title-font font-medium text-2xl text-gray-900">{product[0].price}</span>}
         {product[0].availableQty<=0&& <span className="title-font font-medium text-2xl text-gray-900">Out of stock!</span>}
          
          <button disabled={product[0].availableQty<=0} onClick={()=>buyNow(slug,1,product[0].price,product[0].title,product[0].size,product[0].color)} className="flex ml-4 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded disabled:bg-pink-300">Buy Now</button>
          <button disabled={product[0].availableQty<=0} onClick={()=>{addToCart(slug,1,product[0].price,product[0].title,product[0].size,product[0].color)}} className="flex ml-2 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded disabled:bg-pink-300">Add to Cart</button>
...
export async function getServerSideProps(context){
  let error=null;
    ...
    let variants=await Product.find({title:product[0].title,category:product[0].category})
    let colorSizeSlug={}
